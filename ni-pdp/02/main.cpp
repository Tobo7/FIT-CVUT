...
// Recursive method. Determines whether to continue in DFS branching or when to stop.
    void recursiveSearchParallel(State actualState) {

        if(!isStateValid(actualState)) {
            return;
        }

        if(actualState.getLength() < max(5, inputMatrix->getNodesCount() - 15)) {
            # pragma omp task
            recursiveSearchParallel(actualState.getNextState(GROUP_X));
            # pragma omp task
            recursiveSearchParallel(actualState.getNextState(GROUP_Y));
        } else {
            recursiveSearchSeq(actualState.getNextState(GROUP_X));
            recursiveSearchSeq(actualState.getNextState(GROUP_Y));
        }
...
int main(int argc, char *argv[]) {
    ...
    {
        # pragma omp single
        solutionFinder.recursiveSearchParallel(State(inputMatrix));
    }
