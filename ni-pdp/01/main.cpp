...
// Class implementing the recursive DFS algorithm
class SolutionFinder {
public:
    SolutionFinder(InputMatrix * inputMatrix, int aParam):inputMatrix(inputMatrix),aParam(aParam){};

    // Recursive method. Determines whether to continue in DFS branching or when to stop.
    void recursiveSearch(State actualState) {
        ...
        if(actualState.getGroupCount(GROUP_X) < aParam) {
            recursiveSearch(actualState.getNextState(GROUP_X));
        }

        if (actualState.getGroupCount(GROUP_Y) < inputMatrix->getNodesCount() - aParam) {
            recursiveSearch(actualState.getNextState(GROUP_Y));
        }
    }

    State getFoundMinimumState() {
        return foundMinimumState;
    }

    int getRecursionCalls() {
    ...
    void printSolution() {
        ...
        cout << "Recursive function calls: " << recursiveCalled << endl;
...
int main(int argc, char *argv[]) {
    ...
    SolutionFinder solutionFinder = SolutionFinder(inputMatrix, aParam);

    solutionFinder.recursiveSearch(State(inputMatrix));
