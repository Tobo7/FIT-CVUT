...
int gw_ps_neg_gain (var_info_t varinf, cnt_t cnt, int v) {
    int i, gain=0;
    for (i=0; i<varinf[v].neg_occ_no; i++) {    /* for all clauses where the variable occurs in a negative literal */
        if (cnt[varinf[v].neg_occ[i]] == 0) gain++;
    }
...
int gw_ps_neg_lost (var_info_t varinf, cnt_t cnt, int v) {
    int i, gain=0;
...
int gw_ps_pos_gain (var_info_t varinf, cnt_t cnt, int v) {
    int i, gain=0;
    for (i=0; i<varinf[v].pos_occ_no; i++) {    /* for all clauses where the variable occurs in a positive literal */
        if (cnt[varinf[v].pos_occ[i]] == 0) gain++;
    }
...
int gw_ps_pos_lost (var_info_t varinf, cnt_t cnt, int v) {
    int i, gain=0;
    for (i=0; i<varinf[v].neg_occ_no; i++) {    /* for all clauses where the variable occurs in a negative literal */
        if (cnt[varinf[v].neg_occ[i]] == 1) gain++;
    }
...
/*-----------------------------------------------------------------------------*/
/*      probSAT variable probability calculator                                         */
/*-----------------------------------------------------------------------------*/
double gw_get_var_probability (inst_t* inst, int v, sol_t sol, cnt_t cnt, var_info_t varinf) {

//    double cm = 0;
    double cb = 2.3;

//    int ma = sol[v] ? gw_ps_neg_gain(varinf, cnt, v) : gw_ps_pos_gain(varinf, cnt, v);
    int br = sol[v] ? gw_ps_neg_lost(varinf, cnt, v) : gw_ps_pos_lost(varinf, cnt, v);

    return 1 / pow(br + 1, cb);
//    return pow(ma, cm) / pow(br + 1, cb);
...
/*-----------------------------------------------------------------------------*/
/*      choose a variable in a clause probSAT                                  */
/*-----------------------------------------------------------------------------*/
int gw_pick_var2 (inst_t* inst, cnt_t cnt, int cli, sol_t sol, var_info_t varinf) {
    literal_t* clause;
    clause = inst->body+cli*inst->width;
    ...
    double * probab = (double *) calloc(i, sizeof(double));
    for (int j=0; j<i; j++) {
        probab[j] =  gw_get_var_probability(inst, abs(clause[j]), sol, cnt, varinf);
    }
...
//    printf("sum %.2f ", probabSum);

    double rnd = rng_next_double();
...
//            } else {
                ...
                flipvar = gw_pick_var2(&inst, cnt, ucli, sol, varinf);               /* pick a variable in that clause */
